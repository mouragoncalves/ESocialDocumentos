...
///   ESOCIAL_PROTOCOLO_TESTE  — protocolo de um lote enviado anteriormente (para consulta)
/// </summary>
...
public class ESocialWebServiceIntegrationTests
{
    private static ESocialWebServiceAdapter? CriarAdapter()
    {
        ...
        return new ESocialWebServiceAdapter(
            new CertificadoConfiguration { CaminhoArquivoPfx = certPath, SenhaPfx = certPass },
    ...
    [Fact]
    public async Task ConsultarLote_Homologacao_DeveRetornarStatus()
    {
        var adapter = CriarAdapter();
        if (adapter is null) return;

        var protocolo = Environment.GetEnvironmentVariable("ESOCIAL_PROTOCOLO_TESTE");
        if (string.IsNullOrWhiteSpace(protocolo)) return;

        var retorno = await adapter.ConsultarLoteEventosAsync(protocolo, AmbienteEnvio.Homologacao, CancellationToken.None);

        retorno.Should().NotBeNull();
        retorno.CdResposta.Should().NotBeNullOrEmpty();
    }
}
